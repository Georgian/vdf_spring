...
public final class TokenHandler {
    ...
    public VDFAccount parseUserFromToken(String token) {
        ...
        if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
            try {
                ...
                if (validHash) {
                    final VDFAccount user = fromJSON(userBytes);
    ...
    public String createTokenForUser(VDFAccount user) {
    ...
    private VDFAccount fromJSON(final byte[] userBytes) {
        try {
            return new ObjectMapper().readValue(new ByteArrayInputStream(userBytes), VDFAccount.class);
        } catch (IOException e) {
            throw new IllegalStateException(e);
        }
    }

    private byte[] toJSON(VDFAccount user) {
        try {
